...
dodge.text='Press S or click to start.';
...
dodge.drawEnemy = function(i,x, y) {
    var ind = eElements[i];
   dodge.context.drawImage(images,0, Number(enymy.height * ind), enymy.width,enymy.height, x, y, enymy.width,enymy.height);
}

dodge.drawDiedEnemy = function(i,x, y) {
    var ind = eElements[i];
   dodge.context.drawImage(images,0, Number(enymy.height * (ind+8)), enymy.width,enymy.height, x, y, enymy.width,enymy.height);
}

dodge.drawHero = function() {
   play.setPosition(dodge.position,dodge.height-play.height);
   dodge.context.drawImage(images,0, 0, play.width, play.height, dodge.position,dodge.height-(play.height),play.width,play.height);
  return this;
}
...
dodge.setScore = function(score) {
  $('.scoreBoard').html('<div class="subtext">Fugir</div>' + score).hide().fadeIn(); 
...
dodge.setArrangements = function() {
  ...
  eElements[0]=Math.floor(Math.random()*enymyCount)+1;
  eElements[1]=Math.floor(Math.random()*enymyCount)+1;
  eElements[2]=Math.floor(Math.random()*enymyCount)+1;
